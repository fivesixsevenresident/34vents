...
export default function ProjectsPage() {
 ...
 const sortedProjects = [...filteredProjects].sort((a, b) => b.progress - a.progress);
   ...
   <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
     ...
     <div className="flex flex-wrap gap-2 mb-6">
       <button
         ...
         className={`px-3 py-1 rounded-md text-sm ${filter === 'all' ? 'bg-blue-100 text-blue-800 font-medium' : 'bg-gray-100 hover:bg-gray-200'}`}
       >
         All Projects
       </button>
       {projectsData.projectStatuses.map(status => (
         <button
           ...
           className={`px-3 py-1 rounded-md text-sm ${filter === status.name ? 'bg-blue-100 text-blue-800 font-medium' : 'bg-gray-100 hover:bg-gray-200'}`}
         >
           {status.displayName}
         </button>
       ))}
     ...
     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
       {sortedProjects.map(project => {
         ...
         const completedTasks = project.taskIds.filter(
           taskId => tasksData.tasks.find(task => task.id === taskId)?.status === 'completed'
         ).length;
               ...
               <div className="flex justify-between items-center text-sm text-gray-500">
                 ...
                 <div className="flex -space-x-2">
                   {project.teamMembers.slice(0, 3).map((member, index) => {
                     const user = tasksData.users.find(user => user.username === member);
                     return (
                       <div
                         key={index}
